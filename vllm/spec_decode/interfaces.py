...
@dataclass
class MultiLevelSpeculativeProposals:
    """ 多级推测解码proposal的数据结构 每一级用一个SpeculativeProposals """
    multi_level_proposals: List[SpeculativeProposals] = field(default_factory=list)

    def __repr__(self):
        return (f"MultiLevelSpeculativeProposals("
                f"multi_level_proposals={self.multi_level_proposals})")
