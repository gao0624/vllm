...
class Top1Proposer(SpeculativeProposer):
    ...
    # def _append_proposal_to_execute_model_req(
    #     self,
    #     execute_model_req,
    #     proposal
    # ):
    #     import copy
    #     new_req = copy.deepcopy(execute_model_req)
    #     for i, seq_group in enumerate(new_req.seq_group_metadata_list):
    #         prop_len = proposal.proposal_lens[i].item() if hasattr(proposal.proposal_lens[i], 'item') else proposal.proposal_lens[i]
    #         if prop_len > 0:
    #             prop_tokens = proposal.proposal_token_ids[i][:prop_len].tolist()
    #             for seq in seq_group.seq_data.values():
    #                 if hasattr(seq, "append_tokens"):
    #                   seq.append_tokens(prop_tokens)
    #                 elif hasattr(seq, "input_ids"):
    #                     seq.input_ids.extend(prop_tokens)
    #     return new_req