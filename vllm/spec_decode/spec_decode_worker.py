...
def create_spec_worker(*args, **kwargs) -> "SpecDecodeWorker":
    """
    工厂方法，兼容单级 draft（和你的原始代码完全一致），并自动支持多级 draft。
    - 如 speculative_config.multi_draft_model_configs 存在且非空，走多级 draft。
    - 否则，保持你的原有代码逻辑不变，向后兼容。
    """
    import copy
    ...
    # ----------- 多级 draft 入口 -----------
    multi_draft_configs = getattr(speculative_config, "multi_draft_model_configs", None)
    if multi_draft_configs and len(multi_draft_configs) > 0:
        proposer_workers = []
        for draft_cfg in multi_draft_configs:
            draft_worker_kwargs = copy.deepcopy(kwargs)
            draft_worker_kwargs.update(
                model_config=draft_cfg.model_config,
                parallel_config=draft_cfg.parallel_config,
                ngram_prompt_lookup_max=getattr(draft_cfg, "ngram_prompt_lookup_max", 0),
                ngram_prompt_lookup_min=getattr(draft_cfg, "ngram_prompt_lookup_min", 0),
            )
            # 这里假设你有 SpecDecodeWorker._create_single_draft_worker 或类似工厂方法
            proposer_worker = SpecDecodeWorker._create_single_draft_worker(**draft_worker_kwargs)
            proposer_workers.append(proposer_worker)

        # Target worker
        target_worker_kwargs = copy.deepcopy(kwargs)
        target_worker_kwargs["model_runner_cls"] = TargetModelRunner
        target_worker = Worker(*args, **target_worker_kwargs)
        target_worker.model_runner.disable_logprobs = speculative_config.disable_logprobs

        # 多级 draft worker 实例
        spec_decode_worker = MultiLevelSpecDecodeWorker(
            proposer_workers=proposer_workers,
            target_worker=target_worker,
            disable_by_batch_size=getattr(speculative_config, "speculative_disable_by_batch_size", False),
            draft_token_acceptance_method=getattr(speculative_config, "draft_token_acceptance_method", "default"),
            typical_acceptance_sampler_posterior_threshold=getattr(speculative_config, "typical_acceptance_sampler_posterior_threshold", None),
            typical_acceptance_sampler_posterior_alpha=getattr(speculative_config, "typical_acceptance_sampler_posterior_alpha", None),
            disable_logprobs=getattr(speculative_config, "disable_logprobs", False),
            disable_log_stats=getattr(speculative_config, "disable_log_stats", False),
        )
    ...
    draft_worker_kwargs.update(
        ...
        ngram_prompt_lookup_max=getattr(speculative_config, "ngram_prompt_lookup_max", 0),
        ngram_prompt_lookup_min=getattr(speculative_config, "ngram_prompt_lookup_min", 0),
        # TODO: allow draft-model specific load config if需要
    )
...
class SpecDecodeWorker(LoraNotSupportedWorkerBase):
    ...
    * The scoring forward pass is done without an MQA kernel, which is
    ...
    @staticmethod
    def _create_single_draft_worker(draft_worker_kwargs):
        ...
        if ngram_prompt_lookup_max > 0:
            proposer_worker = NGramWorker(**draft_worker_kwargs)
            proposer_worker.set_ngram_window_size(ngram_prompt_lookup_min, ngram_prompt_lookup_max)
        else:
            model_type = draft_worker_kwargs["model_config"].hf_config.model_type
            if model_type == "mlp_speculator":
                proposer_worker = MLPSpeculatorWorker(**draft_worker_kwargs)
            elif model_type == "medusa":
                proposer_worker = MedusaWorker(**draft_worker_kwargs)
            else:
                draft_tp = draft_worker_kwargs['parallel_config'].tensor_parallel_size
                if draft_tp == 1:
...
class MultiLevelSpecDecodeWorker(LoraNotSupportedWorkerBase):
    """
    支持多级 draft proposer worker 的 SpecDecodeWorker 增强版。
    多级链路：draft1 -> draft2 -> ... -> target
    """
    def __init__(
        self,
        proposer_workers: list,                # [draft1_worker, draft2_worker, ...]
        target_worker: WorkerBase,             # Target scorer worker
        ...
        super().__init__()
        self.proposer_workers = proposer_workers
        ...
        self._disable_logprobs = disable_logprobs
        self._disable_log_stats = disable_log_stats
        # 采样器：与原 SpecDecodeWorker 类似
        if draft_token_acceptance_method == "rejection_sampler":
            self.spec_decode_sampler = RejectionSampler(disable_bonus_tokens=False)
        elif draft_token_acceptance_method == "typical_acceptance_sampler":
            self.spec_decode_sampler = TypicalAcceptanceSampler(
                disable_bonus_tokens=False,
                posterior_threshold=typical_acceptance_sampler_posterior_threshold,
                posterior_alpha=typical_acceptance_sampler_posterior_alpha,
            )
        else:
            raise ValueError(f"Unknown draft_token_acceptance_method: {draft_token_acceptance_method}")
        self._metrics = AsyncMetricsCollector(self.spec_decode_sampler) if metrics_collector is None else metrics_collector
        self.previous_hidden_states: Optional[HiddenStates] = None
    
    @torch.inference_mode()
    def execute_model(self, execute_model_req: Optional[ExecuteModelRequest] = None) -> list:
        """
        多级 draft 主推理流程。
        1. draft1 生成 proposal
        2. draft2 校验 draft1 proposal ...
        3. target 校验最后一级 proposal
        4. 采样/采纳/输出
        """
        if execute_model_req is None:
            return []
        
        t0 = time.perf_counyer()
        # 多级 proposal 递进
        proposal = None
        for idx, worker in enumerate(self.proposer_workers):
            if idx == 0:
                # 头一级直接拿 batch 请求
                proposal = worker.get_spec_proposals(execute_model_req, set())
            else:
                # 后续级拿前一级 proposal
                proposal = worker.get_spec_proposals(execute_model_req, set(), proposal=proposal)
        t1 = time.perf_counter()
        # target 校验最终 proposal
        # 注意：target_worker 通常只需要 score proposals 而非 get proposals
        proposal_scores = self.target_worker.score_proposals(execute_model_req, proposal)
        ...
        return self._create_output_sampler_list(
            ...
            target_logprobs=target_logprobs,
            k=execute_model_req.num_lookahead_slots,
            stage_times=stage_times,   # 可补充真实耗时统计
        )
    ...
    def _verify_tokens(
        ...
        """
        验证最后一层 proposal，采纳或拒绝 token。
        """
        # 下面这段逻辑参考 vLLM SpecDecodeWorker._verify_tokens 实现
        proposal_lens_list = proposals.proposal_lens.tolist()
        (_, spec_indices), (_, non_spec_indices) = split_batch_by_proposal_len(
            seq_group_metadata_list, proposal_lens_list)
        original_indices = spec_indices + non_spec_indices
    ...
    def _create_output_sampler_list(
        ...
        """
        输出采样结构，兼容原有 SpecDecodeWorker 逻辑。
        """
        output = []
        for idx, meta in enumerate(seq_group_metadata_list):
            output.append({
                "seq_group_id": meta.seq_group_id,
                "accepted_token_ids": accepted_token_ids[idx][:k].tolist(),
                "target_logprobs": target_logprobs[idx][:k].tolist() if target_logprobs is not None else None,
                "stage_times": stage_times,
            })
        return output