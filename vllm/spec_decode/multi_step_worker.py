...
from vllm.spec_decode.interfaces import (SpeculativeProposals,
                                         SpeculativeProposer,
                                         MultiLevelSpeculativeProposals)
...
class MultiStepWorker(Worker, ProposerWorkerBase):
    ...
    # ) -> MultiLevelSpeculativeProposals:
    #     """
    #     多级 speculative proposals 生成流程：
    #     1. 第一级 proposal 基于当前 prompt;
    #     2. 第二级 proposal 基于“prompt + 第一级 proposal”;
    #     3. 以此类推，每一级都基于“原始 prompt + 所有前面级别的 proposal”;
    #     4. 返回所有级别 proposal 的列表。
    #     """
    #     proposals_list = []
    #     curr_execute_model_req = execute_model_req
    #     curr_seq_ids = seq_ids_with_bonus_token_in_last_step
    #     for level in range(num_levels):
    #         proposal: SpeculativeProposals = self._proposer.get_spec_proposals(
    #             curr_execute_model_req, curr_seq_ids
    #         )
    #         proposals_list.append(proposal)

    #         # 多级上下文传递
    #         # 1.把当前proposal追加到prompt(序列的output_token_ids)
    #         # 2.构造新的ExecuteModelRequest，包含“原始 prompt + 当前proposal”
    #         # 3. seq_ids_with_bonus_token_in_last_step 通常保持不变（除非有特定逻辑）
    ...
    #     """
    #     把当前 proposal 追加到 execute_model_req 的序列上，构造新请求。
    #     """
    #     new_execute_model_req = execute_model_req.clone()

    #     # 遍历每个 sequence，把 proposal 的 token 追加到 sequence 的 output_token_ids
    #     for seq_group, prop in zip(new_execute_model_req.seq_group_metadata_list, proposal.proposal_token_ids):
    #         for seq_id, seq_data in seq_group.seq_data.items():
    #             # prop: Tensor [proposal_len]
    #             # 追加 proposal token 到 output_token_ids
    #             seq_data.output_token_ids.extend(prop.tolist())
    #             seq_data.update_num_computed_tokens(len(prop))
        
    #     return new_execute_model_req
