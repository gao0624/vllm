...
class DraftModelConfig:
    draft_model_config: Any  # ModelConfig
    draft_parallel_config: Any  # ParallelConfig
    num_speculative_tokens: int
    ngram_prompt_lookup_max: Optional[int] = None
    ngram_prompt_lookup_min: Optional[int] = None
    draft_token_acceptance_method: Optional[str] = None
...
class SpeculativeConfig:
    ...
    def __init__(
        ...
        disable_log_stats: bool,
        multi_draft_model_configs: Optional[List[DraftModelConfig]] = None,
    ):
        ...
        # 多级 draft 支持
        self.multi_draft_model_configs = multi_draft_model_configs

        self._verify_args()

    def is_multilevel(self) -> bool:
        """ 返回是否为多级 """
        return self.multi_draft_model_configs is not None and len(self.multi_draft_model_configs) > 0
    
    @staticmethod
    def from_args(
        draft_model_config: Any = None,
        draft_parallel_config: Any = None,
        ...
        multi_draft_model_configs: Optional[List[dict]] = None,
    ):
        if multi_draft_model_configs:
            # 支持直接传递 DraftModelConfig 或 dict
            draft_cfgs = [
                DraftModelConfig(**cfg) if not isinstance(cfg, DraftModelConfig) else cfg
                for cfg in multi_draft_model_configs
            ]
            # 兼容单级接口，默认用第一个 draft
            first = draft_cfgs[0]
            return SpeculativeConfig(
                draft_model_config=first.draft_model_config,
                draft_parallel_config=first.draft_parallel_config,
                num_speculative_tokens=first.num_speculative_tokens,
                speculative_disable_by_batch_size=speculative_disable_by_batch_size,
                ngram_prompt_lookup_max=first.ngram_prompt_lookup_max,
                ngram_prompt_lookup_min=first.ngram_prompt_lookup_min,
                draft_token_acceptance_method=first.draft_token_acceptance_method,
                ...
                disable_logprobs=disable_logprobs,
                disable_log_stats=disable_log_stats,
                multi_draft_model_configs=draft_cfgs,
            )
        else:
            # 单级兼容
            return SpeculativeConfig(
                ...
                disable_logprobs=disable_logprobs,
                disable_log_stats=disable_log_stats,
                multi_draft_model_configs=None,
            )
    ...
    def _verify_args(self) -> None:
        ...
        if self.is_multilevel():
            for draft_cfg in self.multi_draft_model_configs:
                if draft_cfg.num_speculative_tokens <= 0:
                    raise ValueError("Each draft's num_speculative_tokens must be > 0")
                if not draft_cfg.draft_token_acceptance_method:
                    raise ValueError("Each draft must specify draft_token_acceptance_method")
        else:
            if not self.draft_token_acceptance_method:
                raise ValueError("draft_token_acceptance_method must be set for single draft mode")
            if self.typical_acceptance_sampler_posterior_threshold < 0 or self.typical_acceptance_sampler_posterior_alpha < 0:
                raise ValueError("typical_acceptance_sampler_posterior_threshold and typical_acceptance_sampler_posterior_alpha must be >= 0")
    ...
    def __repr__(self) -> str:
        if self.is_multilevel():
            desc = ', '.join(
                f"level{i+1}:{cfg.draft_model_config.model if hasattr(cfg.draft_model_config, 'model') else str(cfg.draft_model_config)}"
                for i, cfg in enumerate(self.multi_draft_model_configs)
            )
            return f"SpeculativeConfig(MULTILEVEL: {desc})"
        else:
            draft_model = getattr(self.draft_model_config, "model", str(self.draft_model_config))
            return f"SpeculativeConfig({draft_model=}, num_speculative_tokens={self.num_speculative_tokens})"
