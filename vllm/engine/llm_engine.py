...
class LLMEngine:
    ...
    def __init__(
        ...
        # 进行变动  判断是否传入了speculative_config 如果有，则走 create_spec_worker 工厂（适配多级和单级 draft）
        if self.speculative_config is not None:
            self.model_executor = create_spec_worker(
                ...
                step_return_finished_only=step_return_finished_only,
            )
        else:
