...
from vllm.config import (CacheConfig, DecodingConfig, DeviceConfig,
                         ...
                         SpeculativeConfig, TokenizerPoolConfig,DraftModelConfig)
...
class EngineArgs:
    ...
    multi_draft_model_configs: Optional[List[Dict[str, Any]]] = None
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        parser.add_argument(
            '--multi-draft-model-configs',
            type=nullable_str,
            default=None,
            help='JSON字符串,指定多级draft模型配置,'
                '如: \'[{"draft_model_config": {...}, "draft_parallel_config": {...}, "num_speculative_tokens": 4, ...}, ...]\'')
    ...
    def from_cli_args(cls, args: argparse.Namespace):
        ...
        values = {attr: getattr(args, attr) for attr in attrs}

        # 解析多级draft参数
        multi_draft_model_configs = getattr(args, "multi_draft_model_configs", None)
        if multi_draft_model_configs is not None:
            if isinstance(multi_draft_model_configs, str):
                # 允许空字符串和 None
                multi_draft_model_configs = multi_draft_model_configs.strip()
                if multi_draft_model_configs:
                    try:
                        values["multi_draft_model_configs"] = json.loads(multi_draft_model_configs)
                    except Exception as e:
                        raise ValueError(f"multi_draft_model_configs JSON 解析失败: {e}")
                else:
                    values["multi_draft_model_configs"] = None
    ...
    def create_engine_config(self) -> EngineConfig:
        ...
        if self.multi_draft_model_configs:
            # 构造DraftModelConfig列表
            draft_cfgs = []
            for cfg in self.multi_draft_model_configs:
                #draft_model_config/parallel_config 允许传递模型名等基本信息
                draft_model_config = ModelConfig(**cfg["draft_model_config"])
                draft_parallel_config = ParallelConfig(**cfg["draft_parallel_config"])
                draft_cfgs.append(DraftModelConfig(
                    draft_model_config=draft_model_config,
                    draft_parallel_config=draft_parallel_config,
                    num_speculative_tokens=cfg["num_speculative_tokens"],
                    ngram_prompt_lookup_max=cfg["ngram_prompt_lookup_max"],
                    ngram_prompt_lookup_min=cfg["ngram_prompt_lookup_min"],
                    draft_token_acceptance_method=cfg.get("draft_token_acceptance_method"),
                ))
            speculative_config = SpeculativeConfig.from_args(
